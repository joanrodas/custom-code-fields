...
class TextareaField extends Field
{
	...
	public function display_complex(string $parent='') {
		...
		if ($this->type == 'textarea') {
			...
			<p class="form-field _<?= $this->type ?>_field ">
				<label for="_<?= $this->slug ?>"><?= $this->name ?></label>
				<textarea x-cloak class="short" style="<?= $this->height ? 'height:' . $this->height . ';' : 'height: 10rem;' ?>" name="_<?= $this->slug . '[]' ?>" id="<?= $this->slug ?>" placeholder rows="4" cols="20" x-text="entries[tab] ? entries[tab]['<?= $this->slug ?>'] : ''"></textarea>
			</p>
			<?php $input = ob_get_clean();
		}
		echo $input;
